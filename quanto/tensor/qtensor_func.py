...
from .qbits import AWQBitsTensor, QBitsTensor
...
class QTensorLinear(torch.autograd.Function):
    ...
    def forward(ctx, input, other, bias):
        ...
        if isinstance(other, QBitsTensor) and isinstance(other, AWQBitsTensor):
            if type(input) != torch.Tensor:
                input = input.dequantize()
            out_features, in_features = other.shape
            rows = input.numel() // in_features
            other_scaled_zeropoint = -other._zeropoint * other._scale
            output = torch.ops.quanto.gemm(
                input,
                other._data._data,
                other._scale,
                other_scaled_zeropoint,
                rows=rows,
